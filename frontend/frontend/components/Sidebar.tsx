...
import { uploadGuidelines } from '../api/client.ts';
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      if (selectedFile.size > 4 * 1024 * 1024) {
        setToast({ type: 'error', msg: `File too large (${(selectedFile.size / (1024 * 1024)).toFixed(2)} MB). Max 4MB.` });
  ...
  return (
    <div className="w-80 bg-gray-900 border-r border-gray-800 flex flex-col h-full p-6">
      ...
      <div className="mt-auto pt-6 border-t border-gray-800">
        <p className="text-xs text-gray-600 text-center">Veloce.AI Engine v3.0.0 (Modular)</p>
      </div>
    </div>
  );
};
