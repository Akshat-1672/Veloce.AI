...
import { publishToYouTube, syncToGoogleSheets, sendGmailAlert } from '../api/client.ts';
...
export const ResultsPanel: React.FC<ResultsPanelProps> = ({ payload, videoFile, integrations, onOpenIntegrations }) => {
  ...
  const handlePublishYouTube = async () => {
    ...
    try {
      ...
      alert(`Failed to publish to YouTube: ${e.message || 'Unknown error'}`);
    } finally {
  ...
  const handleSyncSheets = async () => {
    ...
    try {
      ...
      alert(`Failed to sync to Google Sheets: ${e.message || 'Unknown error'}`);
    } finally {
      setIsSyncing(false);
    }
  };
